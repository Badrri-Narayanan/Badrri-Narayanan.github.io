(function(global, factory) {
    ...
    function Duration(duration) {
        ...
        this._milliseconds = +milliseconds + seconds * 1e3 + minutes * 6e4 + hours * 3600000;
...
(function(moment) {
    ...
    function buildStringFromValues(yDiff, mDiff, dDiff, hourDiff, minDiff, secDiff) {
        var result = [];
        if (yDiff) result.push(pluralize(yDiff, "year"));
    ...
    moment.preciseDiff = function(d1, d2, returnValueObject) {
        ...
        if (m1.isSame(m2)) {
            if (returnValueObject) return buildValueObject(0, 0, 0, 0, 0, 0, false);
            else return STRINGS.nodiff;
        }
        if (m1.isAfter(m2)) {
            var tmp = m1;
            m1 = m2;
        ...
        if (dDiff < 0) {
            ...
            else dDiff = daysInLastFullMonth + dDiff;
            mDiff--;
        }
        if (mDiff < 0) {
            mDiff = 12 + mDiff;
        ...
        if (returnValueObject) return buildValueObject(yDiff, mDiff, dDiff, hourDiff, minDiff, secDiff, firstDateWasLater);
        else return buildStringFromValues(yDiff, mDiff, dDiff, hourDiff, minDiff, secDiff);
    };
})(moment);

//# sourceMappingURL=Age_calculator_birthday_day_finder.be2a524e.js.map
